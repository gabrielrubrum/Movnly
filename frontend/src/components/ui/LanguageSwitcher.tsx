      ...
      <div className="relative w-full">
        ...
        {open && (
          <>
            ...
            <div className="absolute bottom-full left-0 mb-2.5 w-full bg-surface-1 backdrop-blur-xl rounded-2xl border border-white/10 shadow-2xl p-1.5 z-50">
    ...
    return (
      <div className="relative">
        ...
        {open && (
          <>
            <div className="fixed inset-0 z-40" onClick={closeMenu} />
            <div className="absolute bottom-full left-0 mb-2 w-40 bg-surface-1 backdrop-blur-2xl rounded-xl border border-white/[0.08] shadow-nx-lg p-1.5 z-50 animate-scale-in">
              {locales.map((l) => (
                <button key={l} onClick={() => { setLocale(l); closeMenu(); }}
                  className={cn(
    ...
    return (
      <div className="relative">
        ...
        {open && (
          <>
            <div className="fixed inset-0 z-40" onClick={closeMenu} />
            <div className="absolute bottom-full left-0 mb-2 w-44 bg-surface-1 backdrop-blur-2xl rounded-xl border border-white/[0.08] shadow-nx-lg p-1.5 z-50 animate-scale-in">
              {locales.map((l) => (
                <button key={l} onClick={() => { setLocale(l); closeMenu(); }}
                  className={cn(
                    "w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm transition-all",
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <>
          <div className="fixed inset-0 z-40" onClick={closeMenu} />
          <div className="absolute top-full right-0 mt-2 w-48 bg-surface-1 backdrop-blur-2xl rounded-2xl border border-white/[0.08] shadow-nx-xl p-2 z-50 animate-scale-in">
