...
export class ChatGateway implements OnGatewayConnection, OnGatewayDisconnect {
    ...
    // Client joins their personal room for global notifications
    @SubscribeMessage('join_user_room')
    handleUserJoin(@MessageBody() data: { userId: string }, @ConnectedSocket() client: Socket) {
        if (!data.userId) return;
        client.join(`user:${data.userId}`);
        this.logger.log(`Client ${client.id} joined global notifications for user ${data.userId}`);
    }
    ...
    async handleMessage(
        ...
        // Send Global Notification to the other party
        const booking = await (this.prisma as any).booking.findUnique({
            where: { id: data.bookingId },
        });

        if (booking) {
            const receiverId = booking.passengerId === data.senderId ? booking.driverId : booking.passengerId;
            if (receiverId) {
