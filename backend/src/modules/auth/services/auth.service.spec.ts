...
describe('AuthService', () => {
    ...
    describe('register()', () => {
        it('should register a new user and return user without password', async () => {
            ...
            expect(prisma.user.findUnique).toHaveBeenCalledWith({ where: { email: 'joao@MOVNLY.pt' } });
        ...
        it('should throw ConflictException if email already exists', async () => {
            ...
            await expect(
                service.register({ email: 'joao@MOVNLY.pt', name: 'João', password: '12345678' }),
    ...
    describe('login()', () => {
        it('should return access_token and user on valid credentials', async () => {
            ...
            const result = await service.login({ email: 'joao@MOVNLY.pt', password: 'mypassword123' });

            expect(result).toHaveProperty('access_token');
            expect(result.access_token).toBe('jwt_token_mock_abc123');
            expect(result.user!.email).toBe('joao@MOVNLY.pt');
        ...
        it('should throw UnauthorizedException if user does not exist', async () => {
            ...
            await expect(
                service.login({ email: 'ghost@MOVNLY.pt', password: 'any' }),
        ...
        it('should throw UnauthorizedException if password is incorrect', async () => {
            ...
            await expect(
                service.login({ email: 'joao@MOVNLY.pt', password: 'wrongpassword' }),
            ).rejects.toThrow(UnauthorizedException);
        });
        ...
        it('should include user role in JWT payload', async () => {
            ...
            await service.login({ email: 'joao@MOVNLY.pt', password: 'mypassword123' });
