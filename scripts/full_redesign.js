              ...
              return (
                ...
                >
                  <div className="absolute left-0 top-2 bottom-2 w-0.5 rounded-full"
                    style={{ background: isActive ? "#D4AF37" : isDone ? "#34D399" : "rgba(255,255,255,0.1)" }} />

                  <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ml-2"
                    style={{ background: "rgba(212,175,55,0.07)", border: "1px solid rgba(212,175,55,0.1)" }}>
                    <span className="text-[7px] font-black text-brand-gold/60 group-hover:text-brand-gold transition-colors">{b.reference}</span>
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 mt-0.5">
                      ...
                      <span className="text-white/10">·</span>
                      <span className="text-[8px] font-black uppercase px-1.5 py-0.5 rounded-md" style={{ background: "rgba(212,175,55,0.08)", color: "rgba(212,175,55,0.55)" }}>{b.category}</span>
                    </div>
                  </div>

                  <div className="flex flex-col items-end gap-1.5 flex-shrink-0">
                    <span className="text-sm font-bold tabular-nums text-white/70 group-hover:text-white transition-colors">{formatCurrency(b.totalPrice)}</span>
              ...
              {live.length === 0 ? (
                ...
                <div key={b.id} className="flex items-center gap-3 px-4 py-3 rounded-2xl"
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[9px] text-white/30 mt-0.5">{b.driver?.name || "—"}</p>
                  </div>
            ...
            <div className="p-4 space-y-2">
              {[
                ...
                <div key={label} className="px-4 py-3.5 rounded-2xl"
                  style={{ background: gold ? "rgba(212,175,55,0.06)" : "rgba(255,255,255,0.02)", border: gold ? "1px solid rgba(212,175,55,0.12)" : "1px solid rgba(255,255,255,0.04)" }}>
                  <div className="flex items-center justify-between mb-2">
                    <span className={cn("text-xs font-bold", gold ? "text-brand-gold" : "text-white/40")}>{label}</span>
                    <div className="flex items-center gap-2">
                      <span className={cn("text-sm font-bold tabular-nums", gold ? "text-brand-gold" : "text-white/40")}>{formatCurrency(value)}</span>
                  ...
                  <div className="h-1 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.05)" }}>
                    <motion.div initial={{ width: 0 }} animate={{ width: \`\${pct}%\` }} transition={{ duration: 1.2, delay: 0.5 + i * 0.15 }}
                      ...
                      style={gold ? { background: "linear-gradient(90deg, #D4AF37, #C5A028)", boxShadow: "0 0 6px rgba(212,175,55,0.5)" } : { background: "rgba(255,255,255,0.15)" }}
                    />
                  </div>
                </div>
              ))}
              ...
              {auditLogs.length === 0 ? (
                ...
                return (
                  <div key={i} className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-white/[0.025] transition-all">
                    ...
                    <span className="text-[8px] text-white/15 flex-shrink-0 font-mono tabular-nums">
