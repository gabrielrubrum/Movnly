...
import { Loader2, ChevronRight, ArrowUpRight, Zap, Shield, UserCheck, TrendingUp, Car, Activity, DollarSign, MapPin } from "lucide-react";
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.5 }}
          ...
          <div className="flex items-center justify-between px-7 py-5 border-b border-white/[0.04]">
            <div>
              ...
              <p className="text-[9px] text-white/20 mt-0.5 uppercase tracking-widest font-black">{bookings.length} no sistema</p>
            </div>
            <Link href="/admin/bookings"
              className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white/[0.03] border border-white/[0.06] text-[9px] font-black text-white/30 hover:text-brand-gold hover:border-brand-gold/20 uppercase tracking-widest transition-all">
          ...
          <div>
            {bookings.slice(0, 8).map((b) => (
              <Link key={b.id} href={`/admin/bookings/${b.id}`}
                className="flex items-center gap-4 px-7 py-4 hover:bg-white/[0.025] transition-all group border-b border-white/[0.03] last:border-0">
                <div className="w-11 h-11 rounded-2xl flex items-center justify-center flex-shrink-0 group-hover:scale-105 transition-transform"
                  style={{ background: "linear-gradient(135deg, rgba(212,175,55,0.1) 0%, rgba(212,175,55,0.03) 100%)", border: "1px solid rgba(212,175,55,0.12)" }}>
                  <span className="text-[8px] font-black text-brand-gold/60 group-hover:text-brand-gold transition-colors leading-none text-center">{b.reference}</span>
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 mb-1">
                    <MapPin className="w-3 h-3 text-brand-gold/30 flex-shrink-0" />
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-[9px] text-white/25">{b.passenger.name}</span>
                    <span className="text-white/10">·</span>
                    <span className="text-[9px] text-white/15">{b.pickupDate}</span>
                    <span className="text-white/10">·</span>
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25, duration: 0.5 }}
            ...
            <div className="p-4 space-y-2 max-h-52 overflow-y-auto relative z-10">
              {live.length === 0 ? (
                ...
                <div key={b.id} className="flex items-center gap-3 p-3.5 rounded-2xl transition-all"
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.5 }}
            ...
            <div className="p-4 space-y-3">
              {[
                ...
                <div key={label} className={cn("p-4 rounded-2xl border",
                  gold ? "bg-brand-gold/5 border-brand-gold/[0.12]" : "bg-white/[0.02] border-white/[0.04]")}>
                  <div className="flex items-center justify-between mb-2.5">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className={cn("text-[9px] font-black px-1.5 py-0.5 rounded-lg",
                        gold ? "bg-brand-gold/10 text-brand-gold/60" : "bg-white/5 text-white/20")}>{pct}%</span>
                    </div>
                  </div>
                  <div className="h-1.5 rounded-full overflow-hidden bg-white/[0.05]">
                    <motion.div initial={{ width: 0 }} animate={{ width: `${pct}%` }}
                      ...
                      style={gold
                        ? { background: "linear-gradient(90deg, #D4AF37 0%, #C5A028 100%)", boxShadow: "0 0 8px rgba(212,175,55,0.4)" }
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35, duration: 0.5 }}
            ...
            <div className="divide-y divide-white/[0.03]">
              {drivers?.slice(0, 4).map((d) => (
                <div key={d.id} className="flex items-center gap-4 px-5 py-4 hover:bg-white/[0.025] transition-all group">
                  ...
                  <div className="w-8 h-8 rounded-xl bg-white/[0.03] border border-white/[0.06] flex items-center justify-center group-hover:bg-brand-gold/10 group-hover:border-brand-gold/20 transition-all">
                    <UserCheck className="w-3.5 h-3.5 text-white/15 group-hover:text-brand-gold transition-colors" />
                  </div>
                </div>
              ))}
              {(!drivers || drivers.length === 0) && (
                <p className="text-center text-white/15 text-xs italic py-8">Sem motoristas.</p>
              )}
            </div>
          </motion.div>
                ...
                return (
                  <div key={i} className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-white/[0.025] transition-all">
                    <div className={cn("w-6 h-6 rounded-lg flex items-center justify-center flex-shrink-0 text-[10px] font-black",
                      isLogin ? "bg-emerald-500/10 text-emerald-400" :
                      isDriver ? "bg-brand-gold/10 text-brand-gold" :
                      isStatus ? "bg-blue-500/10 text-blue-400" :
