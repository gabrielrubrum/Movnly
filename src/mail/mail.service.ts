...
export class MailService {
        ...
        <body>
          <div class="wrapper">
            <table class="main">
              <tr>
                <td class="header">
                  <div class="logo">Nex<span>Ride</span></div>
              ...
              <tr>
                <td class="footer">
                  ...
                  <p style="font-size: 8px; color: #111; margin-top: 10px;">NexRide Corporate HQ • Lisbon, Portugal</p>
                </td>
              </tr>
            </table>
          </div>
    ...
    async sendAssignmentEmail(to: string, role: 'DRIVER' | 'PASSENGER', details: any) {
        ...
        const content = `
          ...
          <p style="font-size: 14px; font-weight: 300; italic; color: rgba(255,255,255,0.3);">NexRide Elite: A excelência é o nosso padrão mínimo de operação.</p>
        `;
        return this.sendMail(to, `${subjectLabel} — NexRide Elite`, this.getLuxuryTemplate(content));
    }
    ...
    async sendReceiptEmail(to: string, booking: any, transaction: any) {
        const content = `
          ...
          <p style="font-size: 12px; font-weight: 300; line-height: 1.6; color: rgba(255,255,255,0.4); margin-bottom: 20px;">Este documento serve como comprovativo de pagamento para o serviço de transfer privado NexRide.</p>
          <div style="border-top: 1px solid rgba(255,255,255,0.05); padding-top: 20px;">
            <span style="font-size: 9px; font-weight: 800; color: #222; text-transform: uppercase;">NexRide Elite Technology S.A. | NIF: 500 000 000</span>
          </div>
        `;
    ...
    async sendWithdrawalConfirmationEmail(to: string, amount: number) {
        ...
        `;
        return this.sendMail(to, 'Liquidação de Fundos — NexRide Driver Panel', this.getLuxuryTemplate(content));
    }
}
